...
function Page(props: IndexPageProps) {
  ...
  const worldPageListEntries: PageListEntry[] = props.world
    .sort((a, b) => a.country_code.localeCompare(b.country_code))
    .map((entry) => {
      const url = new URL(
        `${BasePath}./world/${entry.country_code.toLowerCase()}/`,
        getBaseURL(),
      ).href;
      return {
  ...
  const japanPageListEntries: PageListEntry[] = props.japan
    .sort((a, b) => Number.parseInt(a.prefcode) - Number.parseInt(b.prefcode))
    .map((entry) => {
      const url = new URL(
        `${BasePath}./japan/${entry.prefcode.toLowerCase()}/`,
        getBaseURL(),
      ).href;
      return {
        short_name: entry.name_jp,
        long_name: t("japan_long_name", {
          ...entry,
          prefcode_pad: entry.prefcode.padStart(2, "0"),
        }),
        href: url,
      };
    });
