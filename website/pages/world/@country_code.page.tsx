...
export function Page({ entry }: WorldPageProps) {
  const urlSunriseSunset = new URL(
    `${BasePath}./data/ics/world/sunrise-sunset/${entry.country_code.toLowerCase()}-sunrise-sunset.ics`,
    getBaseURL(),
  );
  const urlSunrise = new URL(
    `${BasePath}./data/ics/world/sunrise/${entry.country_code.toLowerCase()}-sunrise.ics`,
    getBaseURL(),
  );
  const urlSunset = new URL(
    `${BasePath}./data/ics/world/sunset/${entry.country_code.toLowerCase()}-sunset.ics`,
    getBaseURL(),
  );
      ...
      >
        <tbody>
          <tr>
            <td>{t("sunrise")}</td>
            <td>
              <CopyableLink href={urlSunrise} />
            </td>
          </tr>
          <tr>
            <td>{t("sunset")}</td>
            <td>
              <CopyableLink href={urlSunset} />
            </td>
          </tr>
          <tr>
            <td>{t("sunrise_sunset")}</td>
            <td>
              <CopyableLink href={urlSunriseSunset} />
            </td>
          </tr>
        </tbody>
      </Table>
